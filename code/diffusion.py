...
xnum = 52 # Grid is 50cm long with 2 nodes for the environment
spacing = 0.01 # Nodes are 1cm apart
num_env_nodes = args.width # Hot end only

num_iters = int(math.ceil(args.time / args.timestep)) # Time to simulate
...
if args.plot:
    for i in range(len(args.plot)):
        ...
        if args.plot[i] == "contour":
            plotting.plot_contour(xnum, width, spacing, solution, title="phi", filename="rod_contour")
        if args.plot[i] == "surface":
            plotting.plot_surface(xnum, width, spacing, solution, title="phi", filename="rod_surface")
